...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8 fade-in" x-data="{ 
    ...
    async handleImageCompression(event) {
        ...
        const options = {
            maxSizeMB: 0.2, // Target max 200KB per image
    ...
    <form action="<?= base_url('admin/properties/store') ?>" method="POST" enctype="multipart/form-data" novalidate class="bg-surface-container-lowest shadow-sm rounded-lg border border-outline-variant p-6 space-y-8">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="title_en" id="title_en" value="<?= old('title_en') ?>" @blur="translateText($event.target.value, 'title_id', 'en|id')" required class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" placeholder="e.g. Luxury Villa in Canggu">
                    <?= $errBox($err) ?>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="title_id" id="title_id" value="<?= old('title_id') ?>" @blur="translateText($event.target.value, 'title_en', 'id|en')" required class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" placeholder="e.g. Vila Mewah di Canggu">
                    <?= $errBox($err) ?>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="tax_price" value="<?= old('tax_price') ?>" required class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>">
                    <?= $errBox($err) ?>
                </div>
                ...
                <div x-show="listingType === 'Rent'" style="display: none;">
                    ...
                    <select name="rental_period" class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" :required="listingType === 'Rent'">
                        <option value="" disabled <?= old('rental_period') ? '' : 'selected' ?>>Select period...</option>
                        <option value="Month" <?= old('rental_period') == 'Month' ? 'selected' : '' ?>>Per Month</option>
                        <option value="Year" <?= old('rental_period') == 'Year' ? 'selected' : '' ?>>Per Year</option>
                    </select>
                    <?= $errBox($err) ?>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="property_tax_number" value="<?= old('property_tax_number') ?>" class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" placeholder="Optional">
                    <?= $errBox($err) ?>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6" x-data="{ 
                ...
                <div>
                    ...
                    <input type="text" name="area_name" id="area_name" value="<?= old('area_name') ?>" class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" placeholder="e.g. Kemang, Canggu">
                    <?= $errBox($err) ?>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="address_line_1" id="address_line_1" value="<?= old('address_line_1') ?>" required class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" placeholder="Street name and number">
                    <?= $errBox($err) ?>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="address_line_2" value="<?= old('address_line_2') ?>" class="w-full px-4 py-3 border rounded focus:ring-1 outline-none <?= $errClass($err) ?>" placeholder="Apartment, suite, unit, building, floor, etc.">
                    <?= $errBox($err) ?>
                </div>
        ...
        <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div><label class="block text-xs mb-1 font-semibold">Beds <span class="text-[#c9302c]">*</span></label><input type="number" name="bed" value="<?= old('bed') ?>" required class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('bed')) ?>"><?= $errBox($getErr('bed')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Baths <span class="text-[#c9302c]">*</span></label><input type="number" name="bath" value="<?= old('bath') ?>" required class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('bath')) ?>"><?= $errBox($getErr('bath')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Total Area (m2) <span class="text-[#c9302c]">*</span></label><input type="number" name="total_area" value="<?= old('total_area') ?>" required class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('total_area')) ?>"><?= $errBox($getErr('total_area')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Usable Area (m2)</label><input type="number" name="usable_area" value="<?= old('usable_area') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('usable_area')) ?>"><?= $errBox($getErr('usable_area')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Land Area (m2)</label><input type="number" name="total_land_area" value="<?= old('total_land_area') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('total_land_area')) ?>"><?= $errBox($getErr('total_land_area')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Year Built</label><input type="number" name="year_built" value="<?= old('year_built') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('year_built')) ?>" placeholder="YYYY"><?= $errBox($getErr('year_built')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Total Floors</label><input type="number" name="total_floors" value="<?= old('total_floors') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('total_floors')) ?>"><?= $errBox($getErr('total_floors')) ?></div>
                <div><label class="block text-xs mb-1 font-semibold">Unit Number</label><input type="text" name="unit_number" value="<?= old('unit_number') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('unit_number')) ?>"><?= $errBox($getErr('unit_number')) ?></div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-3">
                    <label class="block text-xs mb-1 font-semibold">Building / Society Name</label>
                    <input type="text" name="building_society_name" value="<?= old('building_society_name') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('building_society_name')) ?>" placeholder="Name of complex, cluster, or apartment building">
                ...
                <div>
                    ...
                    <input type="number" name="total_parking" value="<?= old('total_parking', 0) ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('total_parking')) ?>">
                ...
                <div class="md:col-span-3">
                    ...
                    <input type="text" name="water_facility" value="<?= old('water_facility') ?>" class="w-full px-3 py-2 border rounded focus:ring-1 outline-none <?= $errClass($getErr('water_facility')) ?>" placeholder="e.g. PDAM, Ground Water, Borehole">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <textarea name="description_en" id="description_en" rows="5" class="w-full px-4 py-3 border rounded focus:ring-1 outline-none resize-y <?= $errClass($err) ?>" placeholder="Provide a detailed description of the property..." @blur="translateText($event.target.value, 'description_id', 'en|id')"><?= old('description_en') ?></textarea>
                    <?= $errBox($err) ?>
                </div>
                <div>
                    <label class="block font-semibold mb-2">Description (ID) <span class="text-[#c9302c]">*</span></label>
                    <?php $err = $getErr('description_id'); ?>
                    <textarea name="description_id" id="description_id" rows="5" class="w-full px-4 py-3 border rounded focus:ring-1 outline-none resize-y <?= $errClass($err) ?>" placeholder="Berikan deskripsi detail tentang properti..." @blur="translateText($event.target.value, 'description_en', 'id|en')"><?= old('description_id') ?></textarea>
                    <?= $errBox($err) ?>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block font-semibold mb-2">Photos (Max 20 images) <span class="text-[#c9302c]">*</span></label>
                    <?php $err = $getErr('property_images'); ?>
                    <input type="file" name="property_images[]" required multiple accept="image/*" @change="handleImageCompression($event)" class="w-full p-2 border rounded p-1 <?= $err ? 'border-[#c9302c] bg-[#fff8f8]' : 'border-outline-variant bg-surface' ?>">
                    <p class="text-xs text-on-surface-variant mt-1">Images will be automatically compressed before upload.</p>
                    <?= $errBox($err) ?>
                </div>
                <div>
                    <label class="block font-semibold mb-2">SHM / Legal Document <span class="text-[#c9302c]">*</span></label>
                    <?php $err = $getErr('shm_document'); ?>
                    <input type="file" name="shm_document" required accept=".pdf,image/*" @change="handleSingleDocCompression($event)" class="w-full p-2 border rounded p-1 <?= $err ? 'border-[#c9302c] bg-[#fff8f8]' : 'border-outline-variant bg-surface' ?>">
                    <?= $errBox($err) ?>
                </div>
            </div>
        </div>
        ...
        <div class="pt-6 border-t flex justify-end">
            <button type="submit" 
                ...
                <span x-text="isCompressing ? 'Compressing Images...' : 'Publish Listing'">Publish Listing</span>
            </button>
        </div>
    </form>
