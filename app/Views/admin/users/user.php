...
<div x-data="{ 
        currentTab: 'active',
        showRoleModal: false, selectedUserId: 0, selectedRole: 1, userName: '', 
        showDeleteModal: false, deleteUrl: '', deleteUserName: '', 
        showRestoreModal: false, restoreUrl: '', restoreUserName: '',
        showForceDeleteModal: false, forceDeleteUrl: '', forceDeleteUserName: '',
        showDocModal: false, docUrl: '', docStatus: '', docDate: '', docUser: '' 
    }" class="flex flex-col gap-unit">
    
    <!-- TABS -->
    <div class="flex border-b border-outline-variant mb-4">
        <button @click="currentTab = 'active'" :class="currentTab === 'active' ? 'border-primary text-primary border-b-2 font-bold' : 'text-on-surface-variant hover:text-on-surface font-semibold'" class="px-6 py-3 transition-colors">
            Active Users
        </button>
        <button @click="currentTab = 'suspended'" :class="currentTab === 'suspended' ? 'border-primary text-primary border-b-2 font-bold' : 'text-on-surface-variant hover:text-on-surface font-semibold'" class="px-6 py-3 transition-colors flex items-center gap-2">
            Suspended Accounts 
            <span class="bg-error-container text-on-error-container py-0.5 px-2 rounded-full text-xs"><?= count($suspendedUsers) ?></span>
        </button>
    </div>
    ...
    <!-- TAB 1: ACTIVE USERS -->
    <div x-show="currentTab === 'active'" class="space-y-4">
        <?php if(!empty($activeUsers) && is_array($activeUsers)): ?>
            <?php foreach($activeUsers as $user): ?>
                ...
                ?>
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 md:px-6 md:py-4 bg-surface border border-outline-variant rounded transition-all">
                    <div class="col-span-1 md:col-span-4 flex items-center gap-stack-md">
                        ...
                        <div>
                            ...
                            <p class="font-caption text-outline"><?= esc($user['email']) ?></p>
                        </div>
                    </div>
            ...
            <?php endforeach; ?>
        <?php else: ?>
            <div class="p-8 text-center bg-surface border border-outline-variant rounded">
                <span class="material-symbols-outlined text-4xl text-outline-variant mb-2">group_off</span>
                <p class="text-on-surface-variant">No active users found in the system.</p>
            </div>
        <?php endif; ?>
    </div>

    <!-- TAB 2: SUSPENDED ACCOUNTS -->
    <div x-show="currentTab === 'suspended'" style="display: none;" class="space-y-4">
        <?php if(!empty($suspendedUsers) && is_array($suspendedUsers)): ?>
            <?php foreach($suspendedUsers as $user): ?>
                <?php 
                    $initials = strtoupper(substr($user['first_name'] ?? 'U', 0, 1) . substr($user['last_name'] ?? '', 0, 1));
                    $roleName = 'Visitor';
                    if ($user['role_id'] == 4) $roleName = 'Admin';
                    if ($user['role_id'] == 3) $roleName = 'Agent';
                    if ($user['role_id'] == 2) $roleName = 'Owner';
                    if ($user['role_id'] == 1) $roleName = 'Buyer';
                ?>
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-4 md:px-6 md:py-4 bg-surface-container-lowest opacity-75 border border-outline-variant rounded transition-all">
                    <div class="col-span-1 md:col-span-4 flex items-center gap-stack-md">
                        <div class="w-10 h-10 rounded-full bg-surface-container flex items-center justify-center text-outline font-bold shrink-0"><?= $initials ?></div>
                        <div>
                            <p class="font-label-md text-on-surface line-through"><?= esc($user['first_name'] . ' ' . $user['last_name']) ?></p>
                            <p class="font-caption text-error font-semibold">Deleted on <?= date('M d, Y', strtotime($user['deleted_at'])) ?></p>
                        </div>
                    </div>
                    <div class="col-span-1 md:col-span-3">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-surface-container-high text-on-surface-variant"><?= $roleName ?></span>
                    </div>
                    <div class="col-span-1 md:col-span-3 text-on-surface-variant">
                        <?= date('M d, Y', strtotime($user['created_date'])) ?>
                    </div>
                    <div class="col-span-1 md:col-span-2 flex justify-end gap-2">
                        <button @click="showRestoreModal = true; 
                                        restoreUrl = '<?= base_url('admin/users/restore/' . $user['id']) ?>'; 
                                        restoreUserName = '<?= esc(addslashes($user['first_name'] . ' ' . $user['last_name'])) ?>';" 
                                class="text-primary hover:bg-primary-container p-2 rounded-full transition-colors" title="Restore User">
                            <span class="material-symbols-outlined">restore_from_trash</span>
                        </button>

                        <button @click="showForceDeleteModal = true; 
                                        forceDeleteUrl = '<?= base_url('admin/users/force-delete/' . $user['id']) ?>'; 
                                        forceDeleteUserName = '<?= esc(addslashes($user['first_name'] . ' ' . $user['last_name'])) ?>';" 
                                class="text-error hover:bg-error-container p-2 rounded-full transition-colors" title="Permanently Delete">
        ...
        <?php else: ?>
            <div class="p-8 text-center bg-surface border border-outline-variant rounded">
                ...
                <p class="text-on-surface-variant">There are currently no suspended or deleted accounts.</p>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div x-show="showRestoreModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-[#1a1c1e]/60 backdrop-blur-sm p-4">
        <div @click.outside="showRestoreModal = false" x-show="showRestoreModal" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" class="bg-surface w-full max-w-sm rounded-xl shadow-2xl border border-outline-variant flex flex-col overflow-hidden">
            <div class="p-6 text-center">
                <div class="w-16 h-16 rounded-full bg-primary-container text-primary flex items-center justify-center mx-auto mb-4">
                    <span class="material-symbols-outlined text-[32px]">restore_from_trash</span>
                </div>
                <h2 class="text-xl font-bold text-on-surface mb-2">Restore User Account</h2>
                <p class="text-sm text-on-surface-variant mb-2">Are you sure you want to restore <span class="font-bold text-on-surface" x-text="restoreUserName"></span>?</p>
                <p class="text-xs text-primary">They will regain access to the platform immediately.</p>
            </div>
            <div class="px-6 py-4 flex justify-between gap-3 bg-surface-container-lowest border-t border-outline-variant">
                <button type="button" @click="showRestoreModal = false" class="flex-1 px-4 py-2 border border-outline-variant text-on-surface-variant rounded font-semibold hover:bg-surface-container transition">Cancel</button>
                <form :action="restoreUrl" method="POST" class="flex-1">
                    <button type="submit" class="w-full px-4 py-2 bg-primary text-on-primary rounded font-semibold hover:opacity-90 transition">Restore</button>
                </form>
            </div>
        </div>
    </div>
    ...
    <div x-show="showForceDeleteModal" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center bg-[#1a1c1e]/60 backdrop-blur-sm p-4">
        <div @click.outside="showForceDeleteModal = false" x-show="showForceDeleteModal" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" class="bg-surface w-full max-w-sm rounded-xl shadow-2xl border border-outline-variant flex flex-col overflow-hidden">
            <div class="p-6 text-center">
                <div class="w-16 h-16 rounded-full bg-error-container text-error flex items-center justify-center mx-auto mb-4">
                    <span class="material-symbols-outlined text-[32px]">warning</span>
                </div>
                <h2 class="text-xl font-bold text-error mb-2">Permanently Delete</h2>
                <p class="text-sm text-on-surface-variant mb-2">Are you sure you want to permanently delete <span class="font-bold text-on-surface" x-text="forceDeleteUserName"></span>?</p>
                <p class="text-xs text-error font-bold">This action cannot be undone and all data will be lost.</p>
            </div>
            <div class="px-6 py-4 flex justify-between gap-3 bg-surface-container-lowest border-t border-outline-variant">
                <button type="button" @click="showForceDeleteModal = false" class="flex-1 px-4 py-2 border border-outline-variant text-on-surface-variant rounded font-semibold hover:bg-surface-container transition">Cancel</button>
                <form :action="forceDeleteUrl" method="POST" class="flex-1">
                    <button type="submit" class="w-full px-4 py-2 bg-error text-on-error rounded font-semibold hover:opacity-90 transition">Yes, Delete</button>
                </form>
            </div>
        </div>
    </div>
