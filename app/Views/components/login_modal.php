<div x-data="{ showAuthModal: false, authTab: 'login', isLoading: false, authError: '' }" 
     @open-auth-modal.window="showAuthModal = true; authError = '';"
    ...
    <div @click.outside="showAuthModal = false" 
        ...
        <div class="p-8 flex flex-col gap-6 mt-2">
            ...
            <div class="flex bg-surface-container-high p-1 rounded-lg">
                <button @click="authTab = 'login'; authError = '';" :class="authTab === 'login' ? 'bg-surface text-primary shadow-sm' : 'text-on-surface-variant'" class="flex-1 py-2 text-[14px] font-bold rounded-md transition-all">Sign In</button>
                <button @click="authTab = 'register'; authError = '';" :class="authTab === 'register' ? 'bg-surface text-primary shadow-sm' : 'text-on-surface-variant'" class="flex-1 py-2 text-[14px] font-bold rounded-md transition-all">Create Account</button>
            </div>

            <div x-show="authError !== ''" x-text="authError" class="bg-error-container text-on-error-container p-3 rounded-lg text-[13px] font-medium" style="display:none;"></div>
            ...
            <form x-show="authTab === 'login'" id="ajaxLoginForm" onsubmit="handleAuthSubmit(event, '<?= base_url('login') ?>')" class="flex flex-col gap-4">
                ...
                <button :disabled="isLoading" class="w-full bg-primary-container text-on-primary font-label-md text-[15px] font-bold py-3 rounded-lg hover:bg-primary transition-colors flex items-center justify-center gap-2 mt-2 shadow-sm disabled:opacity-50" type="submit">
                    <span x-text="isLoading ? 'Authenticating...' : 'Sign In'"></span>
                    <span x-show="!isLoading" class="material-symbols-outlined text-[18px]">login</span>
                </button>
            </form>
            ...
            <button type="button" onclick="handleGoogleLoginMock()" class="w-full bg-surface border border-outline-variant text-on-surface-variant font-label-md text-[14px] font-bold py-3 rounded-lg hover:bg-surface-container-low hover:text-on-surface transition-colors flex items-center justify-center gap-3 shadow-sm">
    ...
    function handleAuthSubmit(e, targetUrl) {
        ...
        .then(data => {
            ...
            if (data.status === 'success') {
                ...
                alpineData.authError = data.message || 'Authentication failed. Please try again.';
            }
        })
        .catch(error => {
            alpineData.isLoading = false;
            alpineData.authError = 'A network error occurred. Please try again.';
        });
    }

    function handleGoogleLoginMock() {
        const alpineData = document.querySelector('[x-data="{ showAuthModal: false"]').__x.$data;
        alpineData.isLoading = true;
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                alpineData.authError = data.message;
            }
        });
    }
