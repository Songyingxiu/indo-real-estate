...
<main class="max-w-[800px] mx-auto px-4 md:px-10 py-12 min-h-[70vh]">
    <div class="mb-8">
        <h1 class="font-headline-lg text-[32px] font-bold text-primary">Complete Your Upgrade</h1>
        <p class="text-on-surface-variant font-body-md">Please complete the manual bank transfer to activate your <?= esc($plan->name) ?> package.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="bg-surface border border-outline-variant rounded-xl p-6 shadow-sm h-fit">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <span class="font-bold text-primary bg-primary-fixed-dim px-2 py-0.5 rounded text-sm"><?= esc($plan->name) ?></span>
            </div>
        ...
        <div class="bg-surface border border-outline-variant rounded-xl p-6 shadow-sm h-fit">
            ...
            <form action="<?= base_url('subscription/upload-proof') ?>" method="POST" enctype="multipart/form-data" class="flex flex-col gap-4">
                ...
                <div>
                    ...
                    <input type="tel" name="phone_number" required placeholder="e.g. 081234567890" value="<?= esc(session()->get('phone_number')) ?>" class="w-full border border-outline-variant rounded px-3 py-2 text-[16px] focus:border-primary focus:ring-1 bg-white outline-none">
                </div>
                ...
                <div class="pt-4 mt-2 border-t border-outline-variant">
                    <button type="submit" class="w-full bg-primary text-on-primary px-6 py-3 rounded font-bold text-[14px] hover:bg-primary-container transition-colors flex items-center justify-center gap-2">
