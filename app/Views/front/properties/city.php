...
<main class="max-w-[1280px] mx-auto px-4 md:px-10 py-10 min-h-[calc(100vh-80px)]">
    <div class="mb-6">
        <h1 class="font-headline-lg text-[32px] font-bold text-on-background">Properties in <?= esc($city['name'] ?? 'this City') ?></h1>
    ...
    <div class="bg-surface border border-outline-variant rounded-lg overflow-hidden shadow-sm h-[400px] mb-10 z-0 relative">
        <div id="cityMap" class="w-full h-full"></div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
        ...
        <?php endforeach; else: ?>
            <p class="col-span-full text-center text-on-surface-variant py-8">No properties found in this city yet.</p>
        <?php endif; ?>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var map = L.map('cityMap').setView([-0.789, 113.921], 5); // Default Indonesia center
        ...
        markersData.forEach(function(m) {
            if(m.latitude && m.longitude) {
                ...
                var cSlug = m.city_name ? m.city_name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') : 'indonesia';
                var tSlug = m.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
                var seoUrl = `<?= base_url('property') ?>/${cSlug}/${tSlug}-${m.id}`;

                marker.bindPopup(`
                    <div class="text-center">
                        <a href="${seoUrl}" class="font-bold text-primary hover:underline">${m.title}</a>
