...
<main class="flex-grow max-w-[1280px] w-full mx-auto px-4 md:px-10 py-12 flex flex-col md:flex-row gap-8 items-start min-h-[75vh]" x-data="userChatApp()">
    ...
    <aside class="w-full md:w-64 flex-shrink-0 sticky top-28 hidden md:block">
        ...
        <nav class="flex flex-col gap-2">
            <a class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary-container text-white font-label-md text-[14px] font-semibold transition-all shadow-sm" href="<?= base_url('user/inbox') ?>">
                <span class="material-symbols-outlined">forum</span> My Inquiries
            </a>
    ...
    <div class="w-full flex flex-col h-[650px]">
        <header class="flex flex-col gap-1 mb-6 shrink-0">
            ...
            <p class="font-body-md text-[16px] text-on-surface-variant">Live chat with property agents.</p>
        </header>
            ...
            <div class="w-1/3 border-r border-outline-variant flex flex-col bg-surface-container-lowest">
                <div class="flex-1 overflow-y-auto custom-scrollbar">
                    <?php if (!empty($threads)): ?>
                        ...
                        <template x-for="thread in threads" :key="thread.inquiry_id">
                            <div @click="loadThread(thread)" 
                                ...
                                <div class="text-[12px] text-on-surface-variant mb-2">Agent: <span x-text="thread.first_name + ' ' + thread.last_name"></span></div>
                    ...
                    <?php else: ?>
                        ...
                        <div class="p-8 text-center text-on-surface-variant text-sm">You haven't contacted any agents yet.</div>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <div class="w-2/3 flex flex-col bg-surface relative">
                <template x-if="!activeThread">
                    <div class="flex-1 flex flex-col items-center justify-center opacity-40">
                        ...
                        <p class="font-bold text-lg text-on-surface">Select a property inquiry</p>
                    </div>
                </template>
                    ...
                    <div class="flex flex-col h-full w-full">
                        <div class="p-4 border-b border-outline-variant bg-surface-container-lowest flex justify-between items-center shadow-sm z-10">
                            <div>
                                ...
                                <p class="text-[13px] text-on-surface-variant">Agent: <span x-text="activeThread.first_name + ' ' + activeThread.last_name"></span></p>
                            </div>
                        </div>
                        ...
                        <div class="p-4 border-t border-outline-variant bg-surface-container-lowest">
                            <form @submit.prevent="sendReply" class="flex gap-3">
                                <textarea x-model="replyText" required rows="2" class="flex-1 px-4 py-3 bg-surface border border-outline-variant rounded-xl outline-none focus:border-primary resize-none text-[14px]" placeholder="Write a reply..." @keydown.enter.prevent="sendReply"></textarea>
